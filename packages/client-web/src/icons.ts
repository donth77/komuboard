...
export const ICONS: Record<string, string> = {
  ...
  // Image (Lucide image) — a framed picture with a sun + mountain.
  image:
    '<rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.1-3.1a2 2 0 0 0-2.8 0L6 21"/>',
  // Plus — the mobile "Insert" launcher (collapses sticky/text/shape/stamp/image on phones).
  plus: '<path d="M12 5v14M5 12h14"/>',
  // Minus — the VR dock's zoom-out key (pairs with plus).
  ...
  // Undo / redo (Lucide undo-2 / redo-2) — on-screen history controls for touch (no keyboard).
...
export const FILLED_ICONS: Record<string, string> = {
  // GitHub mark (Octicons/simple-icons, 24×24) — brand glyph for the "report an issue" link.
  // Solid silhouette, so rendered via iconFilled() with fill=currentColor to theme with the button.
  github:
