...
// Room-name word banks. Big enough that a friendly 3-word id (adjective-animal-noun) has a large
// space (~110³ ≈ 1.3M combinations) so random collisions are rare at early scale; enumeration is
// defended separately by per-IP join rate limiting, NOT by id length (docs/09 SEC-RM). A server-side
// collision check (room index) is the follow-up once concurrent rooms reach the thousands.
const ADJECTIVES = [
  "amber",
  "azure",
  "cobalt",
...
/** CSPRNG word selection — unpredictable + uniform (rejection sampling drops modulo bias). Lists are
 *  all < 256 long, so one random byte per pick suffices. */
function pickWord(list: readonly string[]): string {
  const n = list.length;
  const max = 256 - (256 % n); // largest multiple of n ≤ 256
...
/** A fresh, shareable room id: three friendly words, adjective-animal-noun, e.g. "crimson-otter-maple".
 *  Memorable + typeable + verbal-shareable. The large word space keeps random collisions rare; the
 *  link is made a real (soft) access boundary by per-IP join rate limiting in the worker — NOT by id
 *  length — so it can stay friendly (docs/09 SEC-RM). CSPRNG picks are unpredictable. */
export function randomRoomId(): string {
  return `${pickWord(ADJECTIVES)}-${pickWord(ANIMALS)}-${pickWord(NOUNS)}`;
}
